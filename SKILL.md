...
```bash
# 端点应在「每次会话开始 / 加载本 skill 时」自动执行下面这条（无网则仅警告，不阻断正常使用）
python update_skill.py --auto      # 仅当 GitHub 版本 > 本地才备份并刷新上游文件；本地已最新则什么都不做
python update_skill.py --check     # 只报告是否有更新，不修改任何文件（适合先观望）
```

**怎么做到「加载即触发」**（取决于端点工具是否支持生命周期钩子）：
- 支持 `onLoad` / 前置钩子的工具：把 `python <本skill目录>/update_skill.py --auto` 挂到 skill 加载事件上，会话一开始就静默保新。
- 不支持钩子的工具：在系统提示 / 每次对话开头加一句「先运行 `python update_skill.py --auto` 检查并更新本 skill」，AI 会替你执行。
- 该命令是**幂等且安全的**：远程版本不高于本地时直接跳过；网络不可达时仅打印警告并退出，绝不抛错中断你的正常流程；只有白名单内的上游文件会被刷新，本地优化原样保留。
